...
class OldPassword(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('old_passwords', lazy=True))
...
class Report(db.Model):
    ...
    frequency = db.Column(db.Enum('hourly', 'daily', 'weekly', 'monthly', 'annually', 'manual', name='frequency_enum'))
    time_condition = db.Column(db.Enum('created_since_last_run', 'modified_since_last_run', 'created_all_time', 'created_last_hour',
                                'created_last_day', 'created_last_weekly', 'created_last_month', 'created_last_year', name='time_condition_enum'))
