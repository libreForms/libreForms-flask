    ...
    if request.method == 'POST' and config["smtp_enabled"]:
        ...
        if error is None:
            try: 
                ...
                m = send_mail_async.delay(subject=f'{config["site_name"]} Password Reset', content=content, to_address=email, logfile=log) if config['send_mail_asynchronously'] else mailer.send_mail(subject=f'{config["site_name"]} Password Reset', content=content, to_address=email, logfile=log)
