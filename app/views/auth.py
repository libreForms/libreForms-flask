    ...
    if request.method == 'POST' and config["smtp_enabled"]:
        ...
        if error is None:
            try: 
                ...
                current_app.config['MAILER'](subject=f'{config["site_name"]} Password Reset', content=content, to_address=email, logfile=log).apply_async()
        ...
        if error is None:
            try:
                ...
                if config["enable_email_verification"]:
                    ...
                    current_app.config['MAILER'](subject=f'{config["site_name"]} User Registered', content=f"This email serves to notify you that the user {username} has just been registered for this email address at {config['domain']}.", to_address=email, logfile=log).apply_async()
                ...
                # current_app.config['MAILER'](subject=f"Successfully Registered {username}", content=f"This is a notification that {username} has been successfully registered for libreforms.", to_address=email, logfile=log).apply_async()
                return redirect(url_for("auth.login"))
                    ...
                    if row.group == "" or None:
                        ...
                        try: 
                            ...
                            if config["enable_email_verification"]:
                                ...
                                current_app.config['MAILER'](subject=f'{config["site_name"]} User Registered', content=f"This email serves to notify you that the user {row.username} has just been registered for this email address at {config['domain']}.", to_address=row.email, logfile=log).apply_async()
