        ...
        if request.method == 'POST' and config["allow_anonymous_form_submissions"] and forms['_allow_anonymous_access']:
            ...
            if not error:
                try: 
                    ...
                    m = send_mail_async(subject=f'{config["site_name"]} {form_name} Submission Link', content=content, to_address=email, logfile=log).apply_async() if config['send_mail_asynchronously'] else mailer.send_mail(subject=f'{config["site_name"]} {form_name} Submission Link', content=content, to_address=email, logfile=log)
        ...
        if not signing.verify_signatures(signature, redirect_to='home', 
            ...
            try:
                ...
                if request.method == 'POST':
                    ...
                    m = send_mail_async(subject=subject, content=content, to_address=email, cc_address_list=rationalize_routing_list(form_name), logfile=log).apply_async() if config['send_mail_asynchronously'] else mailer.send_mail(subject=subject, content=content, to_address=email, cc_address_list=rationalize_routing_list(form_name), logfile=log)
